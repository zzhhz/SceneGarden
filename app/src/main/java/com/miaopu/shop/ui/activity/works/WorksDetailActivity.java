...
import com.miaopu.shop.ui.activity.fee.FeeActivity;
import com.miaopu.shop.ui.activity.recommend.adapter.UserCommentAdapter;
import com.miaopu.shop.ui.activity.works.adapter.SingleImageAdapter;
 ...
 * @description: 作品详情页面
 */
public class WorksDetailActivity extends BaseSwipeActivity implements ActionSheetDialog.OnSheetItemClickListener {
     ...
     * 作品id
     */
    public static final String KEY_ID = "work_id";
    ...
    protected void initView() {
        ...
        mSingleImageAdapter = new SingleImageAdapter(mContext, null);
    ...
    protected void initData() {
        ...
        if (TextUtils.isEmpty(id)) {
            id = "e728add9bf554e92931a522a64f69014";
        }
        showDialog();
        RetrofitUtils.Api().getWorkDetail(id).subscribeOn(Schedulers.io()).observeOn(AndroidSchedulers.mainThread()).subscribe(new BaseHandleConsumer<Works>() {
            @Override
            public void onSuccess(Works model) {
                ...
                tv_msg.setText(model.getBrief());
                mSingleImageAdapter.addAll(model.getAttaches());
                mSingleImageAdapter.notifyDataSetChanged();
                tv_comment_count.setText(model.getCommentCount() + "");
                tv_comment_count_2.setText(model.getCommentCount() + " 条评论");
                tv_count_2.setText(" " + model.getCommentCount());
                dismissDialog();
            }
...
//显示软键盘
            ...
            case R.id.iv_fee:
                if (!TextUtils.isEmpty(ShopApplication.getToken())) {
                    if (mSheetDialog == null) {
                        mSheetDialog = new ActionSheetDialog(mContext).addSheetItem("支付宝", ActionSheetDialog.SheetItemColor.Blue, this).addSheetItem("微信", ActionSheetDialog.SheetItemColor.Blue, this).builder().build();
                    }
                    mSheetDialog.onlyShow();
                } else {
    ...
    public void onClick(int which) {
        ...
        intent.putExtra(FeeActivity.KEY_MEMBER_ID, memberId);
        switch (which) {
            case 1:
                intent.putExtra(FeeActivity.KEY_PAY, 0);
                break;
            case 2:
                intent.putExtra(FeeActivity.KEY_PAY, 1);
                break;
        }
